...
export default function CompanyStage({ stages }) {
  ...
  return (
    <div className="w-full xl:w-[50%] flex flex-col items-center rounded-xl px-2 py-5 mx-auto my-5 bg-white relative">
      ...
      <section className="mt-5 w-full border-2 border-black rounded-lg py-4 px-2">
        <div className="flex flex-wrap justify-center">
          <ul className="flex flex-wrap w-full justify-center gap-2">
            {userSkills.map((skill, index) => (
              <li>
                <Skills key={index} bgColor="bg-orange-900">
                  {skill}
                </Skills>
              </li>
            ))}
        ...
        <article className="flex flex-col items-center">
          <h3 className="text-xl underline mt-3 text-green-800 text-center">
        ...
        <div className="text-center font-jetbrains">
          <p className="font-md mt-3 text-magenta font-semibold text-center leading-tight">À partir du 24 mars{start_date}</p>
          <p className="font-md mt-3 text-magenta font-semibold text-center leading-tight">Pour une durée de 2 mois{duration}</p>
        </div>
      </section>
      <div
